...
class VideoService:
    ...
    def _write_video_clip_with_ffmpeg(
        ...
        if (
            result.returncode != 0
            or not os.path.exists(clip_path)
    ...
    def extract_target_embedding(self, video_path: str):
        ...
        while True:
            ...
            normalized_embedding = self.face_service.normalize_embedding(
                faces[0].embedding
            )
    ...
    def _write_video_clip(
        ...
        if ffmpeg_clip_path is None:
            return None

        return self._transcode_clip_for_web(ffmpeg_clip_path)
